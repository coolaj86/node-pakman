(function () {
  ...
  function getModuleLeaf(pkg, prev, requireString, callback) {
    ...
    if (null === prev || !requireString || pkg.name === requireString) {
      ...
      // TODO be a little more robust with lib removal
      // lib/blah -> blah
      // XXX hilib/blah -> hiblah
      // XXX lib/ -> (assume pkg.name)
      modulepath = pkg.name + '/' + leaf.filepath.replace(path.normalize(pkg.lib) + '/', '');
      ...
      paths[0] = pkg.lib;
      getScript(pkg, paths.join('/'), onReadModuleDeps);
      modulepath = requireString;

    } else {

      // check for this package in npm
      modulepath = requireSting;
