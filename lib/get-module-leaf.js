(function () {
  ...
  function getModuleLeaf(pkg, prev, requireString, callback) {
    ...
    if (null === prev || !requireString || pkg.name === requireString) {
      ...
      tmpVar = path.normalize(pkg.moduleRoot + '/' + leaf.filepath).substring(0, pkg.moduleRoot.length);
      if (tmpVar === pkg.moduleRoot) {
          regex = new RegExp('(^|\/)' + escapeRegExp(path.normalize(pkg.lib) + '/'));
          modulepath = pkg.name + '/' + leaf.filepath.replace(regex, function ($) {return $[0];});
      } else {
