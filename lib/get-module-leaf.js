(function () {
  ...
  function getModuleLeaf(pkg, prev, requireString, callback) {
    ...
    if (reIsLocal.exec(dep)) {
      ...
      dep = path.normalize(prev.submodulePath + '/' + dep);
      paths = dep.split('/');
      submoduleName = paths.pop();
      submodulePath = paths.join('/') || '.';
      //getModuleLeaf(pkg, { name: pkg.name, lib: pkg.lib, submodulePath: submodulePath, submoduleName: submoduleName }, onShownModuleDeps);
      ...
      submoduleName = paths.pop();
      submodulePath = paths.join('/') || '.';
      //getModuleLeaf(pkg, { name: pkg.name, lib: pkg.lib, submodulePath: pathname, submoduleName: submoduleName }, onShownModuleDeps);
